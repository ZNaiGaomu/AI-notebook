...
把「随时想到的一句话、一瞬间的灵感、开会时的重要信息、一段录音（可配置转写与润色）、一张截图或实拍、一段视频」**稳定地从手机写进电脑端 Obsidian 库里的 Markdown**。电脑端可接入多家 AI：用对话理解内容，整理并改写文本结构与布局。每一本记录本还能像小插件一样，**用蓝图定义自己的字段与视图**。

**电脑端插件是真相源；Android App 是口袋里的采集端。** 两边共用同一套记录本 / 条目 / 附件约定，而不是各写各的笔记系统——先快速、频繁地记下来，再回电脑做二次 AI 辅助整理，日常与工作都合适。
...
适合两类节奏：**当场必须抓住的碎片**，以及 **回到电脑后才有空做的结构化整理**。下面用五则场景说明，它如何把两者接在同一条笔记线上。

### 故事 A：会还没散，要点已经进了指定条目

会议室里客户连着抛出三件关键事：演示改期、接口负责人、下周验收口径。你来不及打开电脑排版，只在手机 App 里选好「会议本 / 今日客户沟通」，打几句要点，或直接录音——按你在电脑端配好的转写与润色链路，口述也能变成可读文字。

回到工位打开 Obsidian，同一条笔记末尾已经接上刚才的内容；需要时再让浮层助手整理成「决议 / 待办 / 风险」。  
**若只有普通速记 App：** 内容常锁在对方云里，或全部砸进一个巨大收件箱，回电脑还要人工搬家、对不上是哪次会。  
**在这里：** 手机按 **记录本 + 已有条目** 定向追加，电脑里永远是可搜索、可链接的 Markdown。

### 故事 B：灵感、截图、现场画面，不再是另一个 App 里的孤岛

走路时冒出一句产品灵感；展板上随手截图；演示现场录一小段视频。你不想为每一种媒体再开一个「图床 / 语音备忘 / 相册笔记」。

在 App 里选中对应记录本或条目，文字、图片、视频与录音走同一套写入约定：正文进 Markdown，媒体进该条目的附件布局。回电脑用库内搜索、反向链接、图谱，都能找到它们；也可以丢给助手做画面描述或归类。  
**关键点：** 采集发生在手机，**归属与长期管理发生在你自己的库**，不是平行宇宙里的另一套笔记。

### 故事 C：信号差也要记——先攒着，联网再对齐真相源

地下室、高铁、出差酒店：网时有时无，但灵感和会议不会等 Wi‑Fi。把内容先放进 **待发送**；恢复连接后批量推到电脑桥服务，写入你指定的本与条目。

之后在手机「最近」里按 **记录本 → 条目 → 写入历史** 下钻，就能找回「上周那次会的现场图」或某段录音，而不必在扁平时间线里翻半天。手机删的是本地副本与本地最近记录，**不会误删电脑 vault**。

### 故事 D：回电脑，用你自己的 AI 接入口做二次整理

白天用手机保证「先记下」；晚上或工位上才有整块时间。电脑端设置里可接多家 OpenAI 兼容服务商，并按用途（整理 / 助手 / 语音 / 改功能）做有序回退。

打开浮层助手，对象是 **当前这条笔记**：让它理解杂乱速记、改写小标题层级、补结构、描述已插入的图或视频——全部写回库内 Markdown。  
这不是「手机里再嵌一个不可控的云笔记 AI」，而是 **你的 Key、你的路由、你的文件**；采集端负责快，桌面端负责深。

### 故事 E：文献、会议、灵感，结构不同，仍是一套系统

文献本要作者、DOI、摘要；会议本要决议与待办；灵感本也许只要时间流。若为每种本子各装一堆插件，配置会散、手机端更对不齐。

这里每本记录本一份可版本化 **蓝图**（字段、视图、能力开关）。「改功能」用自然语言生成完整蓝图 → Diff 确认 → 提交新版本；回滚功能配置时 **不砍已有正文**。手机与网页入口始终认同一套「本 / 条 / 附件」约定：本子在电脑上长成不同形态，口袋里的采集方式不用换一套。

> 工程上，条目改名会带动附件与对话上传目录一起走、语音写入前确认真实文件存在、打开手机文件时按唯一来源匹配——这些保证「记进去的以后还找得到、播得了」，细节见后文架构与版本说明。
